module User exposing (User, encodeUser, decodeUser, decodeUsersList, getUserByName)
...
getUserByName : Maybe String -> List User -> Maybe User
getUserByName searchstring users = 
  case searchstring of
    Nothing -> Nothing
    Just s ->
      List.head (List.filter (\x -> x.nickname == s) users)